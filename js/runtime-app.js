...
(function () {
  ...
  function rgba(hex, opacityPercent) {
    ...
    var a = Math.max(0, Math.min(100, Number(opacityPercent == null ? 100 : opacityPercent))) / 100;
  ...
  function renderRuntimeAssets() {
    if (window.FormRuntimeAssets && typeof window.FormRuntimeAssets.render === 'function') {
      window.FormRuntimeAssets.render();
      return;
    }

    var logoHost = $('runtimeLogo');
    var logo = schema.assets && schema.assets.headerLogo;
    if (logoHost && logo && logo.enabled !== false && logo.svgText) {
      logoHost.innerHTML = '<img src="' + svgDataUrl(logo.svgText) + '" alt="' + escapeAttribute(logo.alt || 'Logo') + '">';
    }
  }

  function rememberOpenSections() {
    if (!renderingSection('rememberOpenState', true) || !els.form) return;
    els.form.querySelectorAll('details.section[data-section-id]').forEach(function (details) { openSections[details.getAttribute('data-section-id')] = details.open; });
  }

  function cache() {
    els = { title: $('runtimeTitle'), subtitle: $('runtimeSubtitle'), version: $('runtimeVersion'), status: $('runtimeStatus'), form: $('runtimeForm'), list: $('runtimeProtocolList'), take: $('runtimeTakeProtocolButton'), add: $('runtimeNewProtocolButton'), clear: $('runtimeClearDraftButton') };
  }
  ...
  function bindForm() {
    ...
    els.form.querySelectorAll('[data-location-date]').forEach(function (el) {
      el.addEventListener('input', function () {
        var fieldId = el.getAttribute('data-location-date');
        var part = el.getAttribute('data-location-date-part');
        if (!data[fieldId] || typeof data[fieldId] !== 'object' || Array.isArray(data[fieldId])) data[fieldId] = defaultValueForField({ type: 'locationDate' });
        data[fieldId][part] = el.value;
        saveDraft();
      });
    ...
    els.form.querySelectorAll('[data-field-radio]').forEach(function (el) {
      el.addEventListener('change', function () {
        var fieldId = el.getAttribute('data-field-radio');
        data[fieldId] = el.value;
        afterRuntimeFieldChange(fieldId);
      });
    });

    els.form.querySelectorAll('[data-checklist-radio]').forEach(function (el) {
      el.addEventListener('change', function () {
        var fieldId = el.getAttribute('data-checklist-radio');
        var option = el.getAttribute('data-option');
        if (!data[fieldId] || typeof data[fieldId] !== 'object') data[fieldId] = {};
        data[fieldId][option] = data[fieldId][option] || {};
        data[fieldId][option].answer = el.value;
        afterRuntimeFieldChange(fieldId);
      });
    });

    els.form.querySelectorAll('[data-checklist-note]').forEach(function (el) {
      el.addEventListener('input', function () {
        var fieldId = el.getAttribute('data-checklist-note');
        var option = el.getAttribute('data-option');
        if (!data[fieldId] || typeof data[fieldId] !== 'object') data[fieldId] = {};
        data[fieldId][option] = data[fieldId][option] || {};
        data[fieldId][option].note = el.value;
        saveDraft();
      });
    });

    els.form.querySelectorAll('[data-file]').forEach(function (el) {
      el.addEventListener('change', function () {
        var fieldId = el.getAttribute('data-file');
        var nextFiles = Array.prototype.map.call(el.files || [], function (file) { return { name: file.name, type: file.type, size: file.size }; });
        data[fieldId] = (Array.isArray(data[fieldId]) ? data[fieldId] : []).concat(nextFiles);
        el.value = '';
        if (!afterRuntimeFieldChange(fieldId)) renderForm();
      });
    });

    els.form.querySelectorAll('[data-signature]').forEach(function (el) {
      el.addEventListener('click', function () {
        var fieldId = el.getAttribute('data-signature');
        data[fieldId] = data[fieldId] ? '' : 'Unterschrift vorhanden';
        if (!afterRuntimeFieldChange(fieldId)) renderForm();
      });
    });
  }
